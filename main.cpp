...
int squareSize = 20; // Adjust the size as needed
...
// Fill the surface with a solid color (e.g., green)
SDL_FillRect(eti, NULL, SDL_MapRGB(eti->format, 0, 255, 0)); // Green square
	if(eti == NULL) {
	...
	while(!quit) {
		...
		DrawSurface(screen, eti,
				// SCREEN_WIDTH / 2 + (distance/50) * SCREEN_WIDTH / 3,
				// SCREEN_WIDTH / 2 + (direction%2)*(distance/10) * SCREEN_WIDTH,
			    // SCREEN_HEIGHT / 2 + (direction%2-1)*(distance/10) * SCREEN_HEIGHT);
				posX, posY);
		fpsTimer += delta;
		if(fpsTimer > 0.5) {
			fps = frames * 2;
